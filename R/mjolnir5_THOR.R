...
mjolnir5_THOR <- function(lib,cores,tax_dir,tax_dms_name=NULL,obipath="",run_ecotag=T,remove_DMS=T){
  ...
  complete_taxa <- function(fila,info,genus_to_family,family_to_order,class_to_sk,phylum_to_sk,kingdom_to_sk,exceptions) {
    ...
    matrix.data <- left_join(data.frame(X1=columns),matrix.data)
    ...
    if (matrix.data["order_name",2]%in%taxo_names$order_name) {
      matrix.data[c("superkingdom_name","kingdom_name","phylum_name","class_name"),2] <- taxo_names[taxo_names$order_name==matrix.data["order_name",2],c("superkingdom_name","kingdom_name","phylum_name","class_name")]
    }
